 ...
 */
namespace ElementskitVendor\Google\Auth;

use DateTime;
use Exception;
use ElementskitVendor\Firebase\JWT\ExpiredException;
use ElementskitVendor\Firebase\JWT\JWT;
use ElementskitVendor\Firebase\JWT\SignatureInvalidException;
use ElementskitVendor\Google\Auth\Cache\MemoryCacheItemPool;
use ElementskitVendor\Google\Auth\HttpHandler\HttpClientCache;
use ElementskitVendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use ElementskitVendor\GuzzleHttp\Psr7;
use ElementskitVendor\GuzzleHttp\Psr7\Request;
use InvalidArgumentException;
use ElementskitVendor\phpseclib\Crypt\RSA;
use ElementskitVendor\phpseclib\Math\BigInteger;
use ElementskitVendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use ElementskitVendor\SimpleJWT\InvalidTokenException;
use ElementskitVendor\SimpleJWT\JWT as SimpleJWT;
use ElementskitVendor\SimpleJWT\Keys\KeyFactory;
use ElementskitVendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\ElementskitVendor\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\ElementskitVendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('ElementskitVendor\\SimpleJWT\\JWT')) {
    ...
    {
        $class = \class_exists('ElementskitVendor\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
