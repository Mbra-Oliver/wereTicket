...
use ElementskitVendor\Google\Auth\Credentials\InsecureCredentials;
...
abstract class CredentialsLoader implements FetchAuthTokenInterface, UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        ...
        if (\defined('ElementskitVendor\\GuzzleHttp\\ClientInterface::VERSION')) {
    ...
    public static function makeHttpClient(FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], ?callable $httpHandler = null, ?callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \ElementskitVendor\GuzzleHttp\Client($httpClientOptions);
        ...
        $stack = \ElementskitVendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \ElementskitVendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
