...
use ElementskitVendor\Psr\Http\Message\ResponseInterface;
use ElementskitVendor\Psr\Http\Message\UriInterface;
/**
...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('ElementskitVendor\\Firebase\\JWT\\JWT')) {
            return \ElementskitVendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \ElementskitVendor\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('ElementskitVendor\\Firebase\\JWT\\JWT')) {
            return \ElementskitVendor\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \ElementskitVendor\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
